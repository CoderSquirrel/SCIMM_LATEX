...
\subsubsection{Calibracao}
 É classe que contem todos os metodos e variaveis usados no processo de calibração, é dentro dessa classe que são feitas todos os tipos de manipulação em imagem.
 
Os métodos contidos na classe Calibracao são:
	\begin{description}

\item Iniciar: Método onde é instanciado o objeto que faz referencia à \textbf{JanelaPrincipal}, onde tambem se fixa o ID da camera dentro da classe, alem de ser onde se verifica se a mesma esta disponivel.


\item ConfigurarCamera: Método onde a imagem de exibição é ajustando o tamanho da tela a ser usada durante a detecção de objetos alem de ajustar as configurações de brilho e contraste para melhoramento da nitidez da imagem no momento da detecção de objetos.	
	
  \item ReconhecerFundoExtrairObjetos: Método que utiliza o algoritmo de subtração de fundo por mistura gaussiana para identificar o campo. Uma vez que o campo foi identificado o metodo espera a informação do usuario de que já é possivel fazer a extração dos objetos. A extração dos objetos é então feita gerando uma imagem \textit{mascara} que sera usada na detecção dos objetos para esta ser executada com mais precisão, uma vez que os objetos-cor eram os unicos que não fazim parte da imagem inicial, o fundo.
  
 
	\item Calibrar: Método onde é exibido a imagem da camera e quando estiver de acordo com a exigencia do usuario o mesmo inicial a calibração. So apos ser iniciada a calibração o método \textbf{DetectarObjetos} é invocado, apos os objetos de cores serem identificados, apos a detecção dos objtos o metodo invocado é o \textbf{Calcular} onde são analisados os valores de cada pixel de cada objetos e estes classificados de acordo com as cores pré definicas pelo sistemas. Uma vez terminado o processo de calibração é exibida na tela para o usuario os objetos da cor que esta definida no menu de cores.			
	...
	\item ObterPorcentagem: Método simples para devolver a porcentagem de um valor, ao ser informado o valor  e a porcentagem escolhida.
		...
		\item Calcular: Método onde cada pixel pertencente à cada um dos objetos encontrados é analisado, seu valor H é categorizado de acordo com o intervalo de cada cor e então o valor do pixel é adicionado Á cor correspondente.
