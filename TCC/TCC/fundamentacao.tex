...
Quando se trata de detecção de bordas existem dois critérios para essa detecção que devem ser levados em consideração, Taxa de Erro e Localização \cite{Canny:1986, Vale:2002}. 
...
\begin{description}
	\item[Resposta] Para contornar a possibilidade de mais de uma resposta para a mesma borda, ou seja o detector de bordas não deveria identificar múltiplos pixels de borda na qual somente exista um único pixel\cite{Wangenheim:2014, Canny:1986, Vale:2002}.
\end{description}
...
Segundo \citeonline{Azevedo:2003} o universo de cores que podem ser reproduzidas por um sistem é chamado de \textbf{Espaço de Cores}. De acordo com Foley et. al citado por \citeonline{Souto:2003} espaço de cores é um sistema tridimensional de coordenadas, no qual cada eixo refere-se a uma cor primária. A quantidade de cor primária
